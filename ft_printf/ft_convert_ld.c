...
void	itoa_base_ld(long int nbr, char board[64])
{
	int							negative;
	int							size;
...
void	precision_ld(char *tab, t_printf *par, int sub, t_buff *buff)
{
	...
	if (par->precision > ft_strlen(tab))
		precision_ld_sup(tab, par, sub, buff);
...
void	ft_convert_ld(t_printf *par, va_list *ap, t_buff *buff)
{
	...
	if (value == 0)
	{
		if (par->precision_point == 0)
			no_precision_ld_null(par, buff);
		else if (par->precision_point == 1)
		{
			if (par->precision >= 1)
				precision_ld_sup_null(par, buff);
			else if (par->precision < 1)
				precision_ld_inf_null(par, buff);
		}
		return ;
	}
	itoa_base_ld(value, board);
	sub = (board[0] == '-') ? 1 : 0;
	if (par->precision_point == 0)
		no_precision_ld(board, par, sub, buff);
	else if (par->precision_point == 1)
		precision_ld(board, par, sub, buff);
}
