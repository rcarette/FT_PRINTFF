...
/*   Updated: 2017/01/28 13:12:14 by rcarette         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		itoa_base_d(long long int nbr, char board[64])
{
	...
	(negative == 1) ? board[0] = '-' : 0;
}

void		subtraction_null_d(t_printf *par, int sub, int width, t_buff *buff)
{
...
void		precision_d_inf(char *tab, t_printf *par, int sub, t_buff *buff)
{
	...
	par->number_of_character += (ft_strlen(tab) + par->addition + sub
			+ par->space);
	(width > 0) ? par->number_of_character += (width) : 0;
}

void		subtraction_preci(t_printf *par, int sub, int width, t_buff *buff)
{
	print_character(width, ' ', buff);
	(par->space == 1) ? manage_buffer(buff, " ") : 0;
	(par->addition == 1) ? manage_buffer(buff, "+") : 0;
...
void		no_precision_d(char *tab, t_printf *par, int sub, t_buff *buff)
{
	...
	(tab[0] == '-') ? ++tab : 0;
	width = (par->width_field - par->space - par->addition - sub -
			ft_strlen(tab));
	if (par->subtraction == 0)
	{
		subtraction_null_d(par, sub, width, buff);
		manage_buffer(buff, tab);
	}
	else if (par->subtraction == 1)
	{
	...
	par->number_of_character += (ft_strlen(tab) + par->space + par->addition
			+ sub);
	(width > 0) ? par->number_of_character += (width) : 0;
}
...
void		ft_convert_d(t_printf *par, va_list *ap, t_buff *buff)
{
	...
	if (value == 0)
	{
		(par->precision_point == 0) ? no_precision_d_null(par, buff) : 0;
		if (par->precision_point == 1)
		{
			(par->precision >= 1) ? precision_d_sup_null(par, buff) : 0;
			(par->precision < 1) ? precision_d_inf_null(par, buff) : 0;
		}
		return ;
	}
	itoa_base_d(value, board);
	sub = (board[0] == '-') ? 1 : 0;
	(par->precision_point == 0) ? no_precision_d(board, par, sub, buff) : 0;
	(par->precision_point == 1) ? precision_d(board, par, sub, buff) : 0;
}
